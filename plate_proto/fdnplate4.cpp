...
int main(int argc,char**argv){
  ...
  // --- input / diffusion ---
  float bright=E("FDN_BRIGHT",12500.0f); BQ inA; mkLP(inA,bright,0.707);
  ...
  // --- output conditioning ---
  float bodyDb=E("FDN_BODY",2.6f), bodyF=E("FDN_BODYF",250.0f), bodyQ=E("FDN_BODYQ",0.8f);
  BQ bodyL,bodyR; if(bodyDb!=0.0f){ mkPeak(bodyL,bodyF,bodyQ,bodyDb); mkPeak(bodyR,bodyF,bodyQ,bodyDb); }
  float hcDb=E("FDN_HC_DB",-8.0f), hcF=E("FDN_HC_F",6000.0f);
