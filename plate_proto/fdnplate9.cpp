...
int main(int argc,char**argv){
  ...
  if(!hasM){ int Nf=1; while(Nf<N)Nf<<=1; N=Nf; }   // FWHT needs power-of-2; full matrix does not
  float SIZE=E("FDN_SIZE",1.5f); std::vector<int> dl(N);
  for(int i=0;i<N;i++){ int idx=(N>=64)?i:(int)std::round((double)i*63.0/(N-1)); int d=(int)std::round(primeAt(idx)*SIZE); d|=1; dl[i]=d; }
  ...
  for(int i=0;i<N;i++) for(int k=0;k<16;k++) mkPeak(geqd[i][k],GEQ_FC[k],GEQ_Q,GEQ_GUNIT[k]*dl[i]);
  ...
  // optional full orthogonal mixing matrix (else fall back to FWHT)
  ...
  fprintf(stderr,"FDN9 N=%d DC60=%.2f GEQ16band Q=%.2f ER=%dx%.0fms mix=%.2f\n",
