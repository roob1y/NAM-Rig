// fdnplate8 (v2) — two-stage GEQ attenuation filter on the fdnplate7 baseline
...
static int primeAt(int idx){ static const int P[64]={337,389,431,479,523,571,619,661,709,761,811,857,907,953,1009,1061,1103,1153,1201,1259,1301,1361,1409,1459,1511,1559,1607,1657,1709,1759,1811,1867,1913,1973,2017,2069,2113,2161,2213,2267,2309,2357,2411,2467,2521,2579,2621,2677,2729,2777,2833,2887,2939,2999,3041,3089,3137,3187,3251,3301,3347,3391,3457,3461}; return P[idx&63]; }
int main(int argc,char**argv){
  ...
  bool hasM=(getenv("FDN_MATRIX")!=nullptr);
  if(!hasM){ int Nf=1; while(Nf<N)Nf<<=1; N=Nf; }
  ...
  // v2 ATTENUATION FILTER (Jot/Schlecht): broadband gain FLOOR (guarantees loss
  // at every freq incl. DC/Nyquist => stable; sub/air decay at the floor rate) +
  // a two-stage Valimaki-Liski GEQ shaping the residual loss. Command gains are
  // PER-SAMPLE dB (GEQ_GUNIT) scaled by line length dl[i] so all lines share one
  // T60(f). Designed analytically (geq_export.py), NOT random-searched; predicted
  // T60(f) accuracy mean .006/max .029 s vs v1 .016/.124 s.
  float DC60=E("FDN_DC60",4.55f);                 // broadband-floor T60 (s)
  static const double GEQ_FC[16]={62.50,125.00,180.00,250.00,355.00,500.00,710.00,1000.00,1400.00,2000.00,2800.00,4000.00,5600.00,8000.00,11000.00,16000.00};
  static const double GEQ_GUNIT[16]={-1.47554569e-05,2.93910336e-05,-5.50977141e-05,7.43030110e-05,-9.73662486e-05,4.62659789e-05,-8.62206450e-05,-6.49532795e-07,4.46678573e-06,-1.49034486e-04,-4.57545717e-06,-3.29873885e-05,-1.49717889e-04,-1.50558809e-04,-1.42648556e-04,-3.44460151e-04};
  double GEQ_Q=E("FDN_GEQ_Q",0.70f);
  double GUNIT[16]; for(int k=0;k<16;k++) GUNIT[k]=GEQ_GUNIT[k];
  if(const char*gp=getenv("FDN_GUNIT")){ FILE*gf=fopen(gp,"r"); if(gf){ for(int k=0;k<16;k++){ double v; if(fscanf(gf,"%lf",&v)==1) GUNIT[k]=v; } fclose(gf); } }
  double QVEC[16]; for(int k=0;k<16;k++) QVEC[k]=GEQ_Q;
  if(const char*qp=getenv("FDN_QVEC")){ FILE*qf=fopen(qp,"r"); if(qf){ for(int k=0;k<16;k++){ double v; if(fscanf(qf,"%lf",&v)==1) QVEC[k]=v; } fclose(qf); } }
  std::vector<float> g0(N);
  for(int i=0;i<N;i++) g0[i]=(float)std::pow(10.0,-3.0*dl[i]/(DC60*SR));
  std::vector<std::vector<BQ>> geqd(N, std::vector<BQ>(16));
  for(int i=0;i<N;i++) for(int k=0;k<16;k++) mkPeak(geqd[i][k],GEQ_FC[k],QVEC[k],GUNIT[k]*dl[i]);

  // --- input / diffusion ---
  float bright=E("FDN_BRIGHT",6000.0f); BQ inA; mkLP(inA,bright,0.707);
  float drlm=E("FDN_DRV_LM",5.0f), drlmf=E("FDN_DRV_LMF",220.0f), drlmq=E("FDN_DRV_LMQ",0.5f);
  BQ inLM; if(drlm!=0.0f) mkPeak(inLM,drlmf,drlmq,drlm);
  float dhs=E("FDN_DRV_HS",0.0f), dhsf=E("FDN_DRV_HSF",3500.0f);
  BQ inHS; if(dhs!=0.0f) mkHighShelf(inHS,dhsf,dhs);
  float air=E("FDN_AIR",7.0f), airf=E("FDN_AIRF",14000.0f);
  BQ inAIR; if(air!=0.0f) mkHighShelf(inAIR,airf,air);
  ...
  float lcHz=E("FDN_LOWCUT",82.0f); float lcK=lcHz>0?(float)(1.0-std::exp(-2.0*M_PI*lcHz/SR)):0.0f; float hcL=0,hcR=0;

  std::vector<float> M; bool useM=false;
  if(const char*mp=getenv("FDN_MATRIX")){ FILE*mf=fopen(mp,"r"); if(mf){ M.assign((size_t)N*N,0); double v; size_t c=0; while(c<(size_t)N*N && fscanf(mf,"%lf",&v)==1) M[c++]=(float)v; fclose(mf); useM=(c==(size_t)N*N); } }
  std::vector<float> mtmp(N);
  ...
  for(long n=0;n<F;n++){
    float x=0.5f*(in[2*n]+in[2*n+1]); x=inA.proc(x); if(dhs!=0.0f) x=inHS.proc(x); if(air!=0.0f) x=inAIR.proc(x); if(drlm!=0.0f) x=inLM.proc(x); for(int i=0;i<nID;i++) x=idf[i].proc(x);
    ...
    for(int i=0;i<N;i++){ float v=g0[i]*s[i]; for(int k=0;k<16;k++) v=geqd[i][k].proc(v); fb[i]=v; }
    if(useM){ for(int j=0;j<N;j++){ float a=0; const float*row=&M[(size_t)j*N]; for(int i=0;i<N;i++) a+=row[i]*fb[i]; mtmp[j]=a; } for(int i=0;i<N;i++) fb[i]=mtmp[i]; }
    else { fwht(fb.data(),N); for(int i=0;i<N;i++) fb[i]*=invsq; }
    float ing=0.5f; for(int i=0;i<N;i++) line[i].write(fb[i]+sin_[i]*ing*x);
  }
  FILE*go=fopen(argv[2],"wb");fwrite(o.data(),4,F*2,go);fclose(go);
  fprintf(stderr,"FDN11M N=%d DC60=%.2f GEQ16band Q=%.2f ER=%dx%.0fms mix=%.2f\n",
    N,DC60,GEQ_Q,ER_N,ER_MS,erMix);
  return 0;
}
