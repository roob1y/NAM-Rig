...
branch (`model == 2`) so only the Muff shows Sustain/Tone/Volume. **Tone stack reworked
after play-test:** the first pass used the engine see-saw tilt for the Tone knob, which
ACTIVELY boosted the lows +9 dB (huge CCW low end, quiet CW). Replaced with the **real
PASSIVE Big Muff tone stack** — a treble-HP/bass-LP 2nd-order network, derived `H(s,tone)`
