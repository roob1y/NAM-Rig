...
bbd=false (fixed full bandwidth, no repitch). antiAliasHz ≈ 19 kHz (effectively full-band, non-degrading — NOT the old 15 k that darkened repeats). satDrive 0 (no compander, clean). fbCeiling 1.0 (sustains → self-oscillation-capable; the ~19 k in-loop LP loses a hair per pass so it won't run away numerically). Mix law = dry + wet (dry unity). IoStage = transparent buffered (subsonic coupling HPs only; 1 MΩ = DI ref, no shelf; no HF smoothing). Mod knob defaults 0 → DD-7 standard mode is clean; dialing Mod emulates the Modulate mode's gentle chorus.
