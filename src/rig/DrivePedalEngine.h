#pragma once
// DrivePedalEngine — ONE drive pedal (a single MonoBlock). This is the DSP core
// extracted verbatim from the old 3-slot DriveBlock rack: the per-slot state +
// the per-slot processing body are unchanged, so a pedal here is byte-for-byte
// identical to one slot of the old rack. DriveBlock is now a thin wrapper that
// owns 3 of these and runs them in series (see DriveBlock.h). The pedalboard
// POOL (SCOPE_PEDALBOARD §10) allocates these as individual, freely-routable
// board slots. All voicings/models/params/tests carry over unchanged.
//
// (Original DriveBlock rack notes, still accurate for a single pedal:)
// A 3-slot SERIES rack of drive pedals (shared pre, before the
...
class DrivePedalEngine : public MonoBlock
{
public:
    ...
    void setKind(int k)      { kind.store(k); }
    void setDrive(float v)   { drive.store(clamp01(v)); }
    void setTone(float v)    { tone.store(clamp01(v)); }
    void setLevel(float v) { level01.store(clamp01(v)); } // Volume/Level KNOB 0..1 (per-model pot curve, see volFor/volPot)
    void setRange(int r) { range.store(r); } // treble-boost cap switch: 0 Treble/1 Mid/2 Full
    void setOn(bool o) { on.store(o); }            // footswitch (default on)
    void setModel(int m) { model.store(m); }         // model within the category
    void setGateOn(bool o) { gateOn.store(o); }    // fuzz bias-starved gate enable (default on)
    void setMigrateFull(bool full) { migrateFull.store(full); } // RAT hump migration range: false Tight (default) / true Full

    // This pedal contributes to the signal (kind != Off AND footswitch on).
    bool active() const
    {
        return (Kind)kind.load() != Kind::Off && on.load();
    }
    ...
    void prepare(const BlockContext &ctx) override
    {
        ...
        reset();
        io.prepare(mSampleRate); // set IoStage sample rate (reconfigured per voicing on first process)
        mPrepared = true;
    }
    ...
    void process(float *mono, int numSamples) override
    {
        ...
        {
            DrivePedalEngine &s = *this; // the per-slot body below is unchanged (s == this pedal)
            const Kind k = (Kind)s.kind.load();
            if (k == Kind::Off || !s.on.load()) // footswitch off -> bypass (passthrough)
                return;
...
private:
    ...
    // ---- per-pedal state (was DriveBlock::Slot; promoted to members so the
    //      process body below is unchanged with `DrivePedalEngine &s = *this`) ----
