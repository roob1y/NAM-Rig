...
// Ring-buffer strategy: linear buffer with sporadic memmove-rewind.
// A pow2+tail-mirror variant (constant per-block work) was benchmarked against
// this on real hardware (June 2026, clang-cl AVX2, block 64): identical p99/max,
// and the mirror's per-block memcpy cost slightly MORE on mean. The occasional
// rewind memmove (~300 KB worst case) doesn't register on modern memory
// bandwidth, so the simpler linear ring wins.
    ...
    namespace a2_fast
    {
      ...
      namespace
      {
        ...
        public:
          ...
          A2FastModel(std::vector<float> weights, double expected_sample_rate, int oversampling = 1);
        ...
        private:
          struct Layer
          {
            ...
            // Conv1D input history, column-major (Channels rows).
          ...
          // Oversampling dilation for the head taps. 1 = native rate. When oversampling,
          // the head's 16-tap spacing scales by this factor to preserve its temporal
          // window at the higher rate (matches per-layer dilation scaling).
        ...
        // -----------------------------------------------------------------------------
        template <int Channels>
        A2FastModel<Channels>::A2FastModel(std::vector<float> weights, double expected_sample_rate, int oversampling)
            : DSP(/*in_channels=*/1, /*out_channels=*/1, expected_sample_rate)
        {
          const int os = (oversampling < 1) ? 1 : oversampling;
          _head_dilation = os;

          for (int i = 0; i < kNumLayers; i++)
          {
            _layers[i].kernel_size = kKernelSizes[i];
            _layers[i].dilation = kDilations[i] * os;
            _layers[i].max_lookback = (kKernelSizes[i] - 1) * kDilations[i] * os;
        ...
        // -----------------------------------------------------------------------------
        // Ring-write helpers: linear buffer with periodic memmove rewind. When
        // write_pos nears the end of history, memmove the trailing max_lookback
        // cols back to offset 0 and reset write_pos. The rewind cost was measured
        // and does not register against the per-block work (see note at top).
        // -----------------------------------------------------------------------------
        template <int Channels>
        ...
        {
          const int keep = (kHeadKernelSize - 1) * _head_dilation;
        ...
        void A2FastModel<Channels>::_layer_forward_k(Layer &L, const float *cond, int num_frames)
        {
          ...
          // Physical ring position of this block's first frame, offset by
          // `taps_back * D` samples into the past. write_pos is monotonic
          // (linear ring), so the arithmetic is plain.
          const int base = L.write_pos - num_frames;
          auto tap_base_phys = [&](int taps_back)
          { return base - taps_back * D; };
        ...
        void A2FastModel<Channels>::_layer_forward(int layer_idx, const float *cond, int num_frames)
        {
          ...
          case 15:
            ...
            break;
          default:
            throw std::runtime_error("A2FastModel: unexpected kernel_size " + std::to_string(L.kernel_size));
          }
        }
        ...
        void A2FastModel<Channels>::_head_forward(float *output, int num_frames)
        {
          ...
          { return base + f - (kHeadKernelSize - 1 - k) * _head_dilation; };
      ...
      // Build an A2 fast-path model directly with a given oversampling factor.
      // oversampling=1 reproduces the stock model; oversampling=2 scales every
      // layer dilation and the head tap spacing by 2 so the receptive field covers
      // the same real time at the doubled internal rate. Weights are unchanged.
      std::unique_ptr<DSP> build_a2_fast_model(int channels, std::vector<float> weights, double sampleRate,
                                               int oversampling)
      {
        if (channels == 3)
          return std::make_unique<A2FastModel<3>>(std::move(weights), sampleRate, oversampling);
        if (channels == 8)
          return std::make_unique<A2FastModel<8>>(std::move(weights), sampleRate, oversampling);
        throw std::runtime_error("build_a2_fast_model: unsupported channel count " + std::to_string(channels));
      }
